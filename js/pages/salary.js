(function () {
  ...
  function initSalaryPage() {
    ...
    const render = () => {
      ...
      const monthlyDeductions = getDeductionBreakdown(taxableMonthly, dependentValue, childValue);
      const deductionFactor = 1 - Math.min((dependentValue - 1) * 0.015 + childValue * 0.01, 0.08);
      const totalDeduction = monthlyDeductions.total;
      ...
      if (payslipResult) {
        payslipResult.innerHTML = `
          ...
          <div class="premium-breakdown-grid">
            ...
            <article class="premium-breakdown-card">
              <h4>실전형 가정</h4>
              <dl class="premium-breakdown-list compact">
                ...
                <div><dt>공제 합계</dt><dd>${formatWon(realisticMonthlyDeductions.total)}</dd></div>
              </dl>
            </article>
          </div>
        `;
